...
p <- add_argument(p, "--chromlist",
                  help=paste0("List of chromosomes to include, others will be ",
                  "filtered out, new peaks stored in [peak_file].filtered"),
...
withCallingHandlers(
  peakOverlapEnrichment <- enrichPeakOverlap(
...
write_tsv(peakOverlapEnrichment, argv$output_tsv_file)
