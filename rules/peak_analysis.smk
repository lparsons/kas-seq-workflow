...
def get_target_broadpeaks(wildcards):
    query_select = (units["sample"] == wildcards.sample) & (units["unit"] == wildcards.unit)
    query_unit = units[query_select]
    target_units = units[~query_select]
    target_peak_files=expand("results/macs2/{unit.sample}-{unit.unit}_peaks.broadPeak", unit=target_units.itertuples())
